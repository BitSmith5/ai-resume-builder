...
export async function GET(
  ...
  console.log('🚀 MODERN FIXED PDF generation function started - GUARANTEED MODERN TEMPLATE!');
  ...
  try {
    ...
    console.log('🚀 MODERN FIXED - Resume data for PDF generation:', {
    ...
    // FORCE MODERN TEMPLATE - NO EXCEPTIONS!
    const template = 'modern';
    console.log('🚀 MODERN FIXED - Using template for PDF generation:', template);
    console.log('🚀 MODERN FIXED - About to render HTML...');
    const html = renderResumeToHtml(resumeData, template);
    console.log('🚀 MODERN FIXED - HTML rendered successfully, length:', html.length);

    // Return HTML with auto-print functionality
    console.log('🚀 MODERN FIXED - Returning HTML with auto-print functionality');
    const htmlWithAutoPrint = `
      <!DOCTYPE html>
      <html>
      <head>
        <title>${resumeData.title || 'Resume'} - MODERN TEMPLATE</title>
        <meta charset="UTF-8">
        <style>
          @media print {
            body { margin: 0; }
      ...
      <body>
        <button class="auto-print-button" onclick="autoPrint()" id="printBtn">
          🖨️ Auto-Print to PDF (MODERN TEMPLATE)
        </button>
        
        <div class="print-instructions">
          <h3>📄 Save as PDF (MODERN TEMPLATE)</h3>
          <ol>
        ...
        <script>
          function autoPrint() {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                btn.disabled = false;
                btn.textContent = '🖨️ Auto-Print to PDF (MODERN TEMPLATE)';
              }, 2000);
            }, 100);
          }
    ...
    return new NextResponse(htmlWithAutoPrint, {
      headers: {
        ...
        'Content-Disposition': 'inline; filename="resume-modern.html"',
        'Cache-Control': 'no-cache'
      }
    });

  } catch (error) {
    console.error('🚀 MODERN FIXED PDF generation error:', error);
