...
export async function GET(
  ...
  console.log('Service PDF generation function started');
  ...
  try {
    ...
    console.log('Resume data for service PDF generation:', {
    ...
    console.log('Using template for service PDF generation:', template);

    // Generate HTML using the same function as the original PDF route
    console.log('Generating HTML with renderResumeToHtml...');
    const html = renderResumeToHtml(resumeData, template);
    console.log('HTML generated successfully');

    // For now, let's try using a simple approach - return the HTML with instructions
    // In a real implementation, you would send this HTML to a browser service
    console.log('Service PDF generation completed - returning HTML for testing');
    ...
    return NextResponse.json(
      { error: 'Failed to generate service PDF', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
} 