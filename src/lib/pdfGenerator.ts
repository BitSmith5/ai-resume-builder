...
const renderSectionWithPagination = (sectionName: string, resumeData: unknown): string => {
  ...
  // If section has multiple subsections, apply pagination logic
  if (sectionData.hasMultipleSubsections) {
    return `
      <div class="section">
        <div class="section-header">${sectionName}</div>
        ${sectionData.subsections.map((subsection: any, index: number) => `
          <div class="entry ${index === 0 ? 'page-break' : ''}">
    ...
    // Single subsection, use regular rendering
    return renderSection(sectionName, resumeData);
  }
};

// Function to render individual subsections
const renderSubsection = (sectionName: string, subsection: any): string => {
  switch (sectionName) {
    case 'Work Experience':
      return `
        <div class="entry-header">
          <div class="entry-company">${subsection.company}</div>
          <div class="entry-date">${formatDate(subsection.startDate)} - ${subsection.current ? 'Present' : formatDate(subsection.endDate)}</div>
        </div>
        <div class="entry-position body-text">${subsection.position}</div>
        ${subsection.bulletPoints && subsection.bulletPoints.length > 0 ? `
          <div class="bullet-points">
            ${subsection.bulletPoints.map((bullet: any) => `
    ...
    case 'Projects':
      return `
        <div class="entry-title">
          ${subsection.link ? `<a href="${formatUrl(subsection.link)}" class="project-link">${subsection.title}</a>` : subsection.title}
        </div>
        <div class="body-text">${subsection.description}</div>
        ${subsection.technologies && subsection.technologies.length > 0 ? `
          <div class="body-text">Technologies: ${subsection.technologies.join(', ')}</div>
        ` : ''}
      `;
    ...
    case 'Education':
      return `
        <div class="entry-header">
          <div class="entry-title">${subsection.degree} in ${subsection.field}</div>
          <div class="entry-date">${formatDate(subsection.startDate)} - ${subsection.current ? 'Present' : formatDate(subsection.endDate)}</div>
        </div>
        <div class="body-text">${subsection.institution}</div>
        ${subsection.gpa ? `<div class="body-text">GPA: ${subsection.gpa}</div>` : ''}
      `;
    
    case 'Technical Skills':
      if (subsection.skills) {
        return `
          <div class="entry-title">${subsection.title}</div>
          <div class="body-text">${subsection.skills.map((skill: any) => skill.name).join(', ')}</div>
        `;
      } else {
        return `
          <div class="body-text">${subsection.skillName}</div>
        `;
      }
    ...
    case 'Courses':
      return `
        <div class="entry-title">${subsection.title}</div>
        <div class="body-text">${subsection.provider}</div>
      `;
    
    case 'Interests':
      return `
        <div class="body-text">${subsection.name}</div>
      `;
    
    case 'Languages':
      return `
        <div class="body-text">${subsection.name} (${subsection.proficiency})</div>
      `;
    
    case 'Publications':
      return `
        <div class="entry-title">${subsection.title}</div>
        <div class="body-text">${subsection.authors}</div>
        <div class="body-text">${subsection.journal}, ${subsection.year}</div>
      `;
    
    case 'Awards':
      return `
        <div class="entry-title">${subsection.title}</div>
        <div class="body-text">${subsection.issuer}, ${subsection.year}</div>
        ${subsection.description ? `<div class="body-text">${subsection.description}</div>` : ''}
      `;
    
    case 'Volunteer Experience':
      return `
        <div class="entry-header">
          <div class="entry-title">${subsection.position}</div>
          <div class="entry-date">${formatDate(subsection.startDate)} - ${subsection.current ? 'Present' : formatDate(subsection.endDate)}</div>
        </div>
        <div class="body-text">${subsection.organization}</div>
        ${subsection.description ? `<div class="body-text">${subsection.description}</div>` : ''}
      `;
    
    case 'References':
      return `
        <div class="entry-title">${subsection.name}</div>
        <div class="body-text">${subsection.title} at ${subsection.company}</div>
        <div class="body-text">${subsection.email}${subsection.phone ? ` • ${subsection.phone}` : ''}</div>
      `;
...
// Helper function to estimate the height of an entry
function estimateEntryHeight(entry: string, exportSettings: ExportSettings): number {
  const bulletMatches = entry.match(/<div class="bullet-point">/g) || [];
  let height = exportSettings.bodyTextSize * (exportSettings.lineSpacing / 10) * 3; // Base: title + description + spacing
  height += bulletMatches.length * (exportSettings.bodyTextSize * (exportSettings.lineSpacing / 10) + SPACING_CONSTANTS.BULLET_POINT_MARGIN_BOTTOM);
  height += exportSettings.entrySpacing;
  height += SPACING_CONSTANTS.ENTRY_HEADER_MARGIN_BOTTOM + SPACING_CONSTANTS.ENTRY_POSITION_MARGIN_BOTTOM;
...
// Helper function to get section data with subsections
function getSectionData(sectionName: string, resumeData: any): { subsections: any[], hasMultipleSubsections: boolean } | null {
  switch (sectionName) {
    case 'Professional Summary':
      return resumeData.content.personalInfo.summary ? 
        { subsections: [resumeData.content.personalInfo.summary], hasMultipleSubsections: false } : null;
    
    case 'Work Experience':
      return resumeData.workExperience && resumeData.workExperience.length > 0 ? 
        { subsections: resumeData.workExperience, hasMultipleSubsections: resumeData.workExperience.length > 1 } : null;
    
    case 'Projects':
      return resumeData.projects && resumeData.projects.length > 0 ? 
        { subsections: resumeData.projects, hasMultipleSubsections: resumeData.projects.length > 1 } : null;
    
    case 'Education':
      return resumeData.education && resumeData.education.length > 0 ? 
        { subsections: resumeData.education, hasMultipleSubsections: resumeData.education.length > 1 } : null;
    
    case 'Technical Skills':
      if (resumeData.skillCategories && resumeData.skillCategories.length > 0) {
        return { subsections: resumeData.skillCategories, hasMultipleSubsections: resumeData.skillCategories.length > 1 };
      } else if (resumeData.strengths && resumeData.strengths.length > 0) {
        return { subsections: resumeData.strengths, hasMultipleSubsections: resumeData.strengths.length > 1 };
      }
      return null;
    
    case 'Courses':
      return resumeData.courses && resumeData.courses.length > 0 ? 
        { subsections: resumeData.courses, hasMultipleSubsections: resumeData.courses.length > 1 } : null;
    
    case 'Interests':
      return resumeData.interests && resumeData.interests.length > 0 ? 
        { subsections: resumeData.interests, hasMultipleSubsections: resumeData.interests.length > 1 } : null;
    
    case 'Languages':
      return resumeData.languages && resumeData.languages.length > 0 ? 
        { subsections: resumeData.languages, hasMultipleSubsections: resumeData.languages.length > 1 } : null;
    
    case 'Publications':
      return resumeData.publications && resumeData.publications.length > 0 ? 
        { subsections: resumeData.publications, hasMultipleSubsections: resumeData.publications.length > 1 } : null;
    
    case 'Awards':
      return resumeData.awards && resumeData.awards.length > 0 ? 
        { subsections: resumeData.awards, hasMultipleSubsections: resumeData.awards.length > 1 } : null;
    
    case 'Volunteer Experience':
      return resumeData.volunteerExperience && resumeData.volunteerExperience.length > 0 ? 
        { subsections: resumeData.volunteerExperience, hasMultipleSubsections: resumeData.volunteerExperience.length > 1 } : null;
    
    case 'References':
      return resumeData.references && resumeData.references.length > 0 ? 
        { subsections: resumeData.references, hasMultipleSubsections: resumeData.references.length > 1 } : null;
...
// Helper function to calculate text lines based on content length and available width
function calculateTextLines(text: string, fontSize: number, availableWidth: number): number {
  ...
  if (charsPerLine <= 0) return 1; // Fallback to 1 line
  
  // Calculate lines needed
  const lines = Math.ceil(text.length / charsPerLine);
  return Math.max(1, lines); // Minimum 1 line
}

// Helper function to estimate subsection height
function estimateSubsectionHeight(subsection: any, exportSettings: ExportSettings): number {
  const lineHeight = exportSettings.bodyTextSize * (exportSettings.lineSpacing / 10);
  const subHeaderLineHeight = exportSettings.subHeadersSize * (exportSettings.lineSpacing / 10);
  const entrySpacing = exportSettings.entrySpacing;
  
  // Calculate available content width (page width minus margins)
  const pageDimensions = {
  ...
  // Calculate height based on subsection type and content
  if (subsection.company) {
    // Work Experience
    height += subHeaderLineHeight; // Company name
    height += lineHeight; // Position
    height += SPACING_CONSTANTS.ENTRY_HEADER_MARGIN_BOTTOM + SPACING_CONSTANTS.ENTRY_POSITION_MARGIN_BOTTOM;
    
    // Bullet points
    if (subsection.bulletPoints && subsection.bulletPoints.length > 0) {
      subsection.bulletPoints.forEach((bullet: any) => {
        const bulletLines = calculateTextLines(bullet.description, exportSettings.bodyTextSize, contentWidth - 16); // 16px for bullet margin
        height += bulletLines * lineHeight + SPACING_CONSTANTS.BULLET_POINT_MARGIN_BOTTOM;
      });
    }
  } else if (subsection.title && subsection.description) {
    // Projects
    height += subHeaderLineHeight; // Project title
    const descLines = calculateTextLines(subsection.description, exportSettings.bodyTextSize, contentWidth);
    height += descLines * lineHeight;
    
    if (subsection.technologies && subsection.technologies.length > 0) {
      const techText = `Technologies: ${subsection.technologies.join(', ')}`;
      const techLines = calculateTextLines(techText, exportSettings.bodyTextSize, contentWidth);
      height += techLines * lineHeight;
    }
  } else if (subsection.degree && subsection.field) {
    // Education
    height += subHeaderLineHeight; // Degree + field
    height += lineHeight; // Institution
    if (subsection.gpa) {
      height += lineHeight; // GPA
    }
  } else if (subsection.skills) {
    // Technical Skills with categories
    height += subHeaderLineHeight; // Category title
    const skillsText = subsection.skills.map((skill: any) => skill.name).join(', ');
    const skillsLines = calculateTextLines(skillsText, exportSettings.bodyTextSize, contentWidth);
    height += skillsLines * lineHeight;
  } else if (subsection.skillName) {
    // Technical Skills (simple)
    ...
    // References
    height += subHeaderLineHeight; // Name
    height += lineHeight; // Title at company
    height += lineHeight; // Contact info
  } else if (subsection.position && subsection.organization) {
    // Volunteer Experience
    height += subHeaderLineHeight; // Position
    height += lineHeight; // Organization
    if (subsection.description) {
      const descLines = calculateTextLines(subsection.description, exportSettings.bodyTextSize, contentWidth);
      height += descLines * lineHeight;
    }
  } else if (subsection.title && subsection.authors) {
    // Publications
    height += subHeaderLineHeight; // Title
    height += lineHeight; // Authors
    height += lineHeight; // Journal, year
  } else if (subsection.title && subsection.issuer) {
    // Awards
    height += subHeaderLineHeight; // Title
    height += lineHeight; // Issuer, year
    if (subsection.description) {
      const descLines = calculateTextLines(subsection.description, exportSettings.bodyTextSize, contentWidth);
      height += descLines * lineHeight;
    }
  } else if (subsection.title && subsection.provider) {
    // Courses
    height += subHeaderLineHeight; // Course title
    height += lineHeight; // Provider
  } else if (subsection.name) {
    // Interests
    height += lineHeight; // Interest name
  }
...
// Helper function to estimate section height with pagination logic
function estimateSectionHeight(sectionData: { subsections: any[], hasMultipleSubsections: boolean }, exportSettings: ExportSettings): number {
  if (!sectionData || !sectionData.subsections || sectionData.subsections.length === 0) return 0;
  
  const sectionHeaderHeight = exportSettings.sectionHeadersSize * (exportSettings.lineSpacing / 10) + 
                             SPACING_CONSTANTS.SECTION_HEADER_MARGIN_BOTTOM + 
                             SPACING_CONSTANTS.SECTION_HEADER_PADDING_BOTTOM + 
                             1 + // Border width
                             exportSettings.sectionSpacing;
  let totalHeight = sectionHeaderHeight;
  
  // If section has multiple subsections, first subsection needs page break
  if (sectionData.hasMultipleSubsections) {
    // First subsection will be on new page with section header
    totalHeight += estimateSubsectionHeight(sectionData.subsections[0], exportSettings);
    
    // Remaining subsections
    for (let i = 1; i < sectionData.subsections.length; i++) {
      totalHeight += estimateSubsectionHeight(sectionData.subsections[i], exportSettings);
    }
  } else {
    // Single subsection, no special pagination needed
    totalHeight += estimateSubsectionHeight(sectionData.subsections[0], exportSettings);
  }
